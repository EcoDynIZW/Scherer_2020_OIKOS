...
[
  ...
  ;; R0
  inf_size                    ;   [#]            -              number of initially infected individuals
  inf_roam                    ;   [#]            -              number of initially infected roaming males
]
...
[
  ...
  count_inf                   ;   [#]            0              number of infectious indiviuals of this habitat patch
...
to Go
  ...
  if week = 29                  [ Movement_DispFemales ]
                                  ...
                                  ;Host_Reproduction  ;; R0: no reproduction
...
to Init_Parameters
  ...
  set week 0
  ;; R0: pathogen release after a few weeks of movement
  set week_release 3
  ;; R0: estimate one unqiue transmission probability (b_within == b_move)
  set b_within b_move
...
to Disease_Release
  ...
  ;; R0: infect one indiviudal lethally in the middle of landscape to allow spread in all directions
  ...
  [
    ...
    [
      Disease_Infection
      if dem_stat = "dsRoaming" [ set inf_roam inf_roam + 1 ]
      set is_shedding 1
      set epi_stat "esLeth"
      set tick_inf ticks
      set tick_leth ticks + mue
    ]
  ]
...
to Disease_Infection
  ...
  ;; R0: no case fatality rate needed for secondary infections
...
;  if epi_stat = "esSusc" AND age_group = "Adult"    [ set ind_case_fatality case_fatality ^ 2 ]
  ...
  ;; R0: count secondary infections via epi_stat = "esNew", secondary infected pacthes via is_infected 1
;  ifelse random-float 1 <= ind_case_fatality
;  [
    set epi_stat "esNew"
;    set is_shedding 1
;    set tick_leth ticks + 1 + floor (- (mue - 0.5) * ln random-float 1)
...
to Host_Death
  ...
  ;; R0: disease-induced mortality only
...
to Disease_Infection_Move
  ...
  if random-float 1 < (1 - (1 - ind_b_move) ^ (count_inf)) [ Disease_Infection ]
...
to Update
  ...
  ;; R0: stop condition based on survival of initially infected individual only
  if (ticks >= week_release - 1) AND not any? turtles with [is_shedding = 1] [ set DONE 1 ]  ;; R0
  if DONE = 1 [ stop ]
...
to View
  ...
  ask turtles with [epi_stat = "esNew"]
  [
    st
    set color orange
    set size 0.75
  ...
  if ticks >= week_release [ ask patch inf_x inf_y [ set pcolor yellow ] ]
...
"Immune" 1.0 0 -14439633 true "" "plot count turtles with [epi_stat = \"esImm\"]"
"Immune Mat" 1.0 0 -11085214 true "" "plot count turtles with [epi_stat = \"esImmMat\"]"
...
PENS
"Lethally Inf" 1.0 0 -8053223 true "" "plot count turtles with [epi_stat = \"esLeth\"]"
"Transient Inf" 1.0 0 -817084 true "" "plot count turtles with [epi_stat = \"esNew\"]"
...
733
Secondary Inf
count turtles with [epi_stat = \"esNew\"]
17
1
10
...
roaming
roaming
"OFF" "RW" "CRW" "HD" "CD" "DD" "MD" "FD" "HD-CRW"
0
...
@#$#@#$#@
<experiments>
  <experiment name="R0_mue4_base" repetitions="500" runMetricsEveryStep="false">
    <setup>Setup</setup>
    <go>Go</go>
    <timeLimit steps="60"/>
    <exitCondition>DONE = 1</exitCondition>
    <metric>inf_size</metric>
    <metric>inf_roam</metric>
    <metric>count turtles with [epi_stat = "esNew"]</metric>
  ...
  <experiment name="R0_mue4_move" repetitions="500" runMetricsEveryStep="false">
    <setup>Setup</setup>
    <go>Go</go>
    <timeLimit steps="60"/>
    <exitCondition>DONE = 1</exitCondition>
    ...
    <steppedValueSet variable="b_move" first="0.001" step="0.001" last="0.05"/>
    <enumeratedValueSet variable="roaming">
      <value value="&quot;CRW&quot;"/>
      <value value="&quot;HD&quot;"/>
      <value value="&quot;CD&quot;"/>
