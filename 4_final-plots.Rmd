...
## Fig. 4: Multipanel movement + landscape consequences

```{r fig-4-MultiBox-Data}
## data set quarterly
...
df_m4 <- df_quarter %>% 
  group_by(roaming, scenario, run, quarter) %>%  
  #summarize(cell_infectious = mean(cell_infectious / (1250))) %>%
...
```{r fig-4-MultiBox, fig.width = 8.5, fig.height = 11.5}
## a) density of hosts
fig_4a <- ggplot(df_m1, aes(x = scenario, y = avg, 
                            group = type, color = type)) +   
  geom_quasirandom(width = 0.1, dodge.width = 0.85, 
                   alpha = 0.1, size = 0.15, color = "grey75") + 
...
fig_4b <- ggplot(df_m2, aes(x = scenario, y = med, group = type, color = type)) +   
  geom_quasirandom(width = 0.1, dodge.width = 0.85, 
                   alpha = 0.25, size = 0.25, color = "grey75") + 
  geom_boxplot(aes(group = interaction(scenario, type)), 
               width = 0.75, position = position_dodge(0.85), 
               alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
...
fig_4c <- ggplot(df_m3, aes(x = scenario, y = med, group = type, color = type)) +   
  ...
  geom_quasirandom(width = 0.05, dodge.width = 0.95, 
                   alpha = 0.1, size = 0.1, color = "grey75") + 
...
fig_4d <- ggplot(df_m4, aes(x = scenario, y = max)) +   
  geom_quasirandom(width = 0.2, dodge.width = 0.175, 
                   alpha = 0.25, size = 0.25, color = "grey60") + 
...
## e) affected patches
fig_4e <- ggplot(df_m5, aes(x = scenario, y = max, group = scenario)) +   
  geom_quasirandom(width = 0.2, dodge.width = 0.175, 
                   alpha = 0.25, size = 0.25, color = "grey60") + 
...
fig_4f <- ggplot(df_m6, aes(x = scenario, y = avg, 
                            group = type, color = type)) +   
  geom_quasirandom(width = 0.1, dodge.width = 0.85, 
                   alpha = 0.1, size = 0.15, color = "grey75") + 
  geom_boxplot(aes(group = interaction(scenario, type)), 
...
((fig_4 <- fig_4a + fig_4b + fig_4c + fig_4d + fig_4e + fig_4f) *
...
## final panel
if (save == T) { ggsave("./plots/final/Fig4_multi_box.png", 
                        width = 8.5, height = 11.5, dpi = 750) }
```
...
## final panel
(fig_4_b <- (fig_4b_a + fig_4b_b + fig_4b_c + fig_4b_d + fig_4b_e + fig_4b_f) *
...
if (save == T) { ggsave("./plots/final/Fig4b_multi_median.png", 
                        width = 8.5, height = 11.5, dpi = dpi) }
```


## Fig. 5: Probabilities of invasion success and persistence per case fatality risk

```{r fig-5-pers-cfr-tile, fig.width = 8, fig.height = 6.3}
df_5 <- df_infmean_plots %>% 
  mutate(roaming = factor(roaming, labels = lab_roam_n)) %>% 
  group_by(roaming, scenario, cfr) %>%
  summarize(
    pers_mean = mean(pers),
...
fig_5a <- ggplot(df_5, aes(cfr, fct_rev(scenario))) +
...
fig_5b <- ggplot(df_5, aes(cfr, fct_rev(scenario))) +
...
if (save == T) { ggsave("./plots/final/Fig5_pers_cfr_tile.png", width = 8, height = 6.3, dpi = dpi) }
```
...
ggsave("./plots/final/FigS3_move_weights.png", 
       width = 12, height = 6.15, dpi = dpi)
```
...
if (save == T) { ggsave("./plots/final/FigS7_pers_cfr_all.png", 
                        width = 7, height = 8, dpi = dpi) }
```
...
if (save == T) { ggsave("./plots/final/FigS8_time2extinction.png", 
                        width = 12, height = 7.1, dpi = dpi) }
```
...
if (save == T) { ggsave("./plots/final/FigS9_dynamics_g+m.png", 
                        width = 9, height = 6.7, dpi = dpi) }
```
...
if (save == T) { ggsave("./plots/final/FigS10_density_pre+post.png", 
                        width = 9, height = 5, dpi = 750) }
```
