...
```{r fig-5-MultiBox, fig.width = 8.5, fig.height = 11.5}
## data seth
...
df_m2 <- df_all %>% 
  ...
  group_by(roaming, scenario, type, run) %>% 
  summarize(avg = median(visits_mean, na.rm = T)) %>% 
  group_by(roaming, scenario, type) 
  
fig_5b <- ggplot(df_m2, aes(x = scenario, y = avg, group = type, color = type)) +   
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.75, position = position_dodge(0.85), 
               alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
...
df_m4 <- df_quarter %>% 
  group_by(roaming, scenario, run, quarter) %>%  
  summarize(cell_infectious = mean(cell_infectious / (1250 - patches_0))) %>% 
  group_by(roaming, scenario, run) %>%  
  summarize(mean = max(cell_infectious)) %>% 
  gather(type, max, -roaming, -scenario, -run)

fig_5d <- ggplot(df_m4, aes(x = scenario, y = max)) +   
  geom_boxplot(aes(group = scenario), width = 0.6,color = "#cc5d00", 
               alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
...
fig_5e <- ggplot(df_m5, aes(x = scenario, y = max)) +   
  geom_boxplot(aes(group = scenario), width = 0.6,color = "#a01e1e", 
               alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.25)) +
  labs(x = "Landscape scenario", y = "\nProportion of\naffected patches", 
...
fig_5f <- ggplot(df_m6, aes(x = scenario, y = avg, group = type, color = type)) +   
  ...
  theme(axis.text.x = element_text(size = 9.5, angle = 33, hjust = 1, vjust = 1),
        axis.title.x = element_text(size = 11),
        strip.text = element_blank(),
        strip.background = element_blank())
...
## final panel
if (save == T) { ggsave("./plots/final/Fig5_multi_box.png", width = 8.5, height = 11.5, dpi = 750) }
```

```{r fig-5-multi-median, fig.width = 8.5, fig.height = 11.5, eval = F, include = F}
## a) density of hosts
fig_5b_a <- df_m1  %>% 
  group_by(roaming, scenario, type) %>% 
...
fig_5b_d <- df_m4 %>% 
  ...
  ggplot(aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
    ...
    facet_grid(~ roaming) +
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.25)) +
    scale_color_manual(values = c("#cc5d00"), guide = F) + 
...
if (save == T) { ggsave("./plots/final/Fig5b_multi_median.png", width = 8.5, height = 11.5, dpi = dpi) }
```
...
```{r fig-s10-MultiBoxJitter, fig.width = 10, fig.height = 13}
## a) density of hosts
...
## final panel
if (save == T) { ggsave("./plots/final/FigS10_multi_raw.png", width = 10, height = 13, dpi = 350) }
```

## Fig. S11: "Density pre- and post release"
...
df_all %>% 
  ...
  ggplot(aes(x = scenario, y = males, group = period, color = period)) +   
    geom_boxplot(aes(group = interaction(scenario, period)), width = 0.75, position = position_dodge(0.85), 
                 alpha = 0, outlier.alpha = 0.25, size = 0.4) + 
    facet_grid(~ roaming) +
    scale_y_log10()+
    scale_color_manual(values = c("#7aa6e5", "#1a3357"), name = "Period:") + 
    labs(x = "Landscape scenario", y = "Variance in density of moving hosts\n(log-scale)") +
    theme(axis.text.x = element_text(size = 12, angle = 33, hjust = 1, vjust = 1),
          legend.title = element_text(size = 11.5, face = "plain"),
          legend.text = element_text(size = 10),
          legend.position = "bottom",
          strip.text = element_text(size = 12))

if (save == T) { ggsave("./plots/final/FigS11_density_pre+post.png", width = 9, height = 5, dpi = 750) }
```
