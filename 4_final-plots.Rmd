...
fig_1b <- ggplot() +   
  geom_line(data = filter(df_1b, pers == ">10"), aes(week_pathogen, ind_inf, group = run, 
            color = "<10"), size = 0.3, alpha = 0.4) +
  geom_line(data = filter(df_1b, pers == "<10"), aes(week_pathogen, ind_inf, group = run, 
            color = ">10"), size = 0.3, alpha = 0.4) +
  geom_line(data = df_1b, aes(week_pathogen, meanInf), color = "black", size = 0.35) +
  geom_rug(data = filter(df_1b, pers == "<10"), aes(duration_weeks), 
           sides = "b", alpha = 0.4, size = 0.3) +
  scale_x_continuous(breaks = seq(0, 520, by = 52), limits = c(0, 520), labels = weeks_only) + 
  scale_y_continuous(labels = scales::comma_format()) +
  scale_color_manual(values = c(">10" = "grey20", "<10" = "tan1"), name = NULL, labels = labs) +
  theme(legend.position = c(0.75, 0.85), 
        legend.box = "horizontal",
        legend.background = element_rect(fill = "transparent"),
        plot.background = element_rect(fill = "transparent",colour = NA)) +
  labs(x = "Week since pathogen release", y = "Infected individuals")
```
...
fig_1c <- ggplot(filter(df_1c, duration_quarter != 48), aes(duration_quarter / 4, prop)) + 
  ...
  geom_vline(xintercept = 2.625, linetype = "dotted", color = "grey30", size = 0.4) +
  geom_vline(xintercept = 5.125, linetype = "dotted", color = "grey30", size = 0.4) +
  geom_vline(xintercept = 7.625, linetype = "dotted", color = "grey30", size = 0.4) +
...
if (save == T) { ggsave("./plots/final/Fig1_workflow.png", width = 11.7, height = 2.5, dpi = dpi) }
```


## Fig. 2: Persistence probabilities

```{r fig-2-pers-groups, fig.width = 13.5, fig.height = 7.5}
df_2 <- df_infmean_plots %>% 
  filter(cfr == cfr_filter)

## per movement strategy
fig_2_roam <- df_2 %>% 
  group_by(roaming) %>%
  summarize(
    pers_mean = mean(pers),
  ...
  ungroup() %>% 
    ...
    geom_bar(aes(y = pers_mean, fill = roaming), stat = "identity", position = "dodge", width = 0.7) + 
    geom_bar(aes(y = inv_mean), stat = "identity", position = "dodge", width = 0.7, fill = "transparent") + 
    facet_wrap(~ group) +
    ...
    scale_color_manual(values = c("grey80", "grey55", "grey30"), guide = F) + 
    scale_fill_manual(values = c("grey80", "grey55", "grey30"), name = "Movement strategy") + 
    labs(x = NULL, y = "Persistence probability", tag = "a  ")
...
fig_2_scen <- df_2 %>% 
  ...
  ggplot(aes(x = scenario, color = scenario)) +   
    geom_bar(aes(y = pers_mean, fill = scenario), stat = "identity", position = "dodge", width = 0.7) + 
    geom_bar(aes(y = inv_mean), stat = "identity", position = "dodge", width = 0.7, fill = "transparent") + 
    facet_wrap(~ group) +
    expand_limits(y = c(0, 1)) +
    theme(legend.position = "none",
          ...
          axis.title.x = element_text(size = 14),
          axis.title.y = element_text(size = 14)) +
    scale_color_viridis_d(option = "inferno", begin = 0.15, end = 0.85, direction = -1, guide = F) +
    scale_fill_viridis_d(option = "inferno", begin = 0.15, end = 0.85, direction = -1, guide = F) +
    labs(x = NULL, y = "Persistence probability", tag = "b  ")
...
fig_2_detail <- df_2 %>% 
  ...
  ggplot(aes(x = scenario, color = scenario)) + 
    ...
    scale_color_viridis_d(option = "inferno", begin = 0.15, end = 0.85, direction = -1, guide = F) +
    labs(x = NULL, y = "Persistence probability", tag = "c  ")

(fig_2 <- plot_spacer() + fig_2_roam + fig_2_scen + fig_2_detail + 
            plot_layout(nrow = 2, heights = c(1, 1.5), widths = c(1, 3)))

if (save == T) { ggsave("./plots/final/Fig2_pers_groups.png", width = 13.5, height = 7.5, dpi = dpi) }
```


## Fig. 3: Temporal outbreak dynamics

```{r fig-3-temp-dynamics, fig.width = 8.9, fig.height = 6.9}
...
(fig_3 <- df_trans %>% 
  ...
  ggplot(aes(quarter / 4, avg)) +
    ...
    geom_vline(aes(xintercept = 2.75), linetype = "dashed", color = "grey60", size = 0.3) +
...
if (save == T) { ggsave("./plots/final/Fig3_dynamics_b+w.png", width = 8.9, height = 6.9, dpi = dpi) }
```


## Fig. 4: Probabilities of invasion success and persistence per case fatality risk

```{r fig-4-pers-cfr-tile, fig.width = 8, fig.height = 6.3}
df_4 <- df_infmean_plots %>% 
  mutate(roaming = factor(roaming, labels = lab_roam_n)) %>% 
  group_by(roaming, scenario, cfr) %>%
  summarize(
    pers_mean = mean(pers),
...
fig_4a <- ggplot(df_4, aes(cfr, fct_rev(scenario))) +
  geom_tile(aes(fill = pers_mean)) +
  facet_wrap(~ roaming) + 
...
fig_4b <- ggplot(df_4, aes(cfr, fct_rev(scenario))) +
  geom_tile(aes(fill = inv_mean)) +
  facet_wrap(~ roaming) + 
  scale_fill_viridis(option = "cividis", name = "Probability", limits = c(0, 1)) +
  scale_x_discrete(expand = c(0, 0)) +
  scale_y_discrete(expand = c(0, 0)) +
  labs(x = "Case fatality ratio", y = NULL, tag = "b", 
       title = "Successful invasion (outbreak duration > 2.5 years)")
...
if (save == T) { ggsave("./plots/final/Fig4_pers_cfr_tile.png", width = 8, height = 6.3, dpi = dpi) }
```


## Fig. 5: Multipanel movement + landscape consequences

```{r fig-5-MultiBox, fig.width = 8.5, fig.height = 10.5}
## data seth
df_quarter <- df_inf_plots %>% 
  ...
  dplyr::select(
    ...
    week_inf_max,  ## d: week of maximum spread
    trans_w, trans_b, patches_0  ## f: within- and between-patch trans
  ) %>% 
  mutate(
...
fig_5a <- ggplot(df_m1, aes(x = scenario, y = avg, group = type, color = type)) +   
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.75, position = position_dodge(0.85), 
               alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 130), breaks = seq(0, 100, by = 50)) +
...
df_m2 <- df_quarter %>% 
  group_by(roaming, scenario, run, quarter) %>% 
  summarize(
    visits_mean = median(visits_mean, na.rm = T)
  ) %>% 
  gather(type, avg, -roaming, -scenario, -run, -quarter) %>% 
  group_by(roaming, scenario, type)

fig_5b <- ggplot(df_m2, aes(x = scenario, y = avg)) +   
  geom_boxplot(aes(group = scenario), width = 0.6,
               color = "#696969", alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
...
hom <- df_m3  %>% 
  group_by(roaming, scenario) %>% 
  summarize(mean = mean(avg, na.rm = T))

fig_5c <- ggplot(df_m3, aes(x = scenario, y = avg, group = type, color = type)) +   
  geom_path(data = hom, aes(x = scenario, y = mean, group = roaming), 
            color = "grey60", linetype = "dashed", size = 0.5) +
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.7, 
               position = position_dodge(0.85, preserve = "single"), 
               fill = "white", outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 80), breaks = seq(0, 75, by = 25)) +
  scale_color_manual(values = c("#8cd085", "#3c8834", "#1c3f18"), name = "Habitat quality:") + 
  labs(x = "Landscape scenario", y = "\nContact rates of\n infected ranging males", 
...
## d) week of maximum spread
df_m4 <- df_quarter %>% 
  group_by(roaming, scenario, run) %>%  ## no quarter since unique values per run
  summarize(
    max = max(week_inf_max)
  ) %>% 
  gather(type, max, -roaming, -scenario, -run)

fig_5d <- ggplot(df_m4, aes(x = scenario, y = max)) +   
  geom_boxplot(aes(group = scenario), width = 0.6,
               color = "#a01e1e", alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 55), breaks = seq(0, 50, by = 25)) +
  labs(x = "Landscape scenario", y = "\nWeek of maximum\npathogen spread", 
...
## e) maximum number of infectious cells
df_m5 <- df_inf_plots %>% 
  filter(cfr == cfr_filter) %>% 
  mutate(
    roaming = factor(roaming, labels = lab_roam_n),
    cell_infectious = cell_infectious / (1250 - patches_0)
  ) %>% 
  dplyr::select(run, roaming, scenario, cell_infectious) %>% 
  group_by(roaming, scenario, run) %>%  ## no quarter since unique values per run
  summarize(
    max = max(cell_infectious)
  ) %>% 
  gather(type, max, -roaming, -scenario, -run)

fig_5e <- ggplot(df_m5, aes(x = scenario, y = max)) +   
  geom_boxplot(aes(group = scenario), width = 0.6,
               color = "#cc5d00", alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
...
df_m6 <- df_quarter %>% 
  ...
  mutate(
    ...
    type = ifelse(scenario == "Homogeneous", glue::glue("{type}_hom"), glue::glue("{type}_het"))
  ) %>% 
  filter(type != "b_hom") %>% 
  mutate(type = factor(type, levels = c("b_het", "w_het", "w_hom"),
                             labels = c("Between-cluster", "Within-cluster", "Homogeneous")))

fig_5f <- ggplot(df_m6, aes(x = scenario, y = avg, group = type, color = type)) +   
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.75, position = position_dodge(0.85), 
               alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.5)) +
  scale_color_manual(values = c("#d48d09", "#580080", "#8a8a8a"), name = "Transmission:") + 
  labs(x = "Landscape scenario", y = "\nProportion of\ninfections", 
       tag = "f", title = NULL, subtitle = NULL) +
  theme(axis.text.x = element_text(size = 11, angle = 33, hjust = 1, vjust = 1),
        axis.title.x = element_text(size = 11),
        legend.title = element_text(size = 11, face = "plain"),
        legend.text = element_text(size = 10),
        strip.text = element_blank(),
...
## final panel
if (save == T) { ggsave("./plots/final/Fig5_multi_box.png", width = 8.5, height = 10.5, dpi = 750) }
```

```{r fig-5-multi-median, fig.width = 9, fig.height = 11, eval = F, include = F}
## a) density of hosts
fig_5b_a <- df_m1  %>% 
  group_by(roaming, scenario, type) %>% 
...
fig_5b_b <- df_m2  %>% 
  ...
  ggplot(aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
    ...
    geom_point(position = position_dodge(width = 0.55), size = 2.5) +
    facet_grid(~ roaming) +
    scale_y_continuous(limits = c(0, 12.5), breaks = seq(0, 12, by = 6)) +
    scale_color_manual(values = c("#696969"), guide = F) + 
    scale_shape_manual(values = c(19, 17, 15), guide = F) + 
    labs(x = "Landscape scenario", y = "\nVisited patches\nper week", 
         tag = "b", title = NULL, subtitle = NULL) +
    theme(axis.ticks.x = element_blank(),
          axis.text.x = element_blank(),
...
fig_5b_c <- df_m3  %>% 
  ...
  ggplot(aes(x = scenario, y = avg)) +   
    ...
    scale_color_manual(values = c("#8cd085", "#3c8834", "#1c3f18"), name = "Habitat quality:") + 
    scale_shape_manual(values = c(19, 17, 15), name = "Habitat quality:") + 
    labs(x = "Landscape scenario", y = "\nContact rates of\n infected ranging males", 
         tag = "c", title = NULL, subtitle = NULL) +
    theme(axis.ticks.x = element_blank(),
          axis.text.x = element_blank(),
...
fig_5b_d <- df_m4 %>% 
  group_by(roaming, scenario, type) %>% 
  summarize(avg = median(max, na.rm = T)) %>% 
  ggplot(aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
    geom_path(position = position_dodge(width = 0.55), size = 1, alpha = 0.4) +
    geom_point(position = position_dodge(width = 0.55), size = 2.5) +
    facet_grid(~ roaming) +
    scale_y_continuous(limits = c(0, 55), breaks = seq(0, 50, by = 25)) +
    scale_color_manual(values = c("#a01e1e"), guide = F) + 
    scale_shape_manual(values = c(19), guide = F) + 
    labs(x = "Landscape scenario", y = "\nWeek of maximum\npathogen spread", 
         tag = "d", title = NULL, subtitle = NULL) +
    theme(axis.ticks.x = element_blank(),
          axis.text.x = element_blank(),
...
fig_5b_e <- df_m5 %>% 
  ...
  ggplot(aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
    ...
    facet_grid(~ roaming) +
    scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.5)) +
    scale_color_manual(values = c("#e66800"), guide = F) + 
    scale_shape_manual(values = c(19), guide = F) + 
    labs(x = "Landscape scenario", y = "\nMaximum proportion\nof infectious patches", 
         tag = "e", title = NULL, subtitle = NULL) +
    theme(axis.ticks.x = element_blank(),
          axis.text.x = element_blank(),
          axis.title.x = element_blank(),
...
fig_5b_f <- df_m6 %>% 
  ...
  ggplot(aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
    ...
    geom_point(position = position_dodge(width = 0.55), size = 2.5) +
    facet_grid(~ roaming) +
    scale_y_continuous(limits = c(-0.1, 1.1), breaks = seq(0, 1, by = 0.5)) +
    scale_color_manual(values = c("#d48d09", "#580080", "#8a8a8a"), name = "Transmission:") + 
    scale_shape_manual(values = c(19, 17, 15), name = "Transmission:") + 
    labs(x = "Landscape scenario", y = "\nProportion of\ninfections", 
         tag = "f", title = NULL, subtitle = NULL) +
    theme(axis.text.x = element_text(size = 11, angle = 33, hjust = 1, vjust = 1),
          axis.title.x = element_text(size = 11),
...
## final panel
(fig_5b <- (fig_5b_a + fig_5b_b + fig_5b_c + fig_5b_d + fig_5b_e + fig_5b_f) *
    theme(axis.text.y = element_text(size = 10),
          axis.title.y = element_text(size = 10.5),
          plot.margin = margin(1, 1, 1, 1)) +
    plot_layout(ncol = 1))

if (save == T) { ggsave("./plots/final/Fig5b_multi_median.png", width = 8.5, height = 10.5, dpi = dpi) }
```
...
p_CRW <- ggplot(d, aes(x, y, height = height, grpup = 1, fill = x)) +
  geom_ridgeline_gradient(color = NA) +
  scale_fill_viridis_c(guide = "none", option = "cividis") +
  coord_flip() +
  labs(x = NULL, y = "", title = "Correlated\nrandom walk\n") +
...
p_HD <- ggplot(HD, aes(x, qual)) +
  ...
  labs(x = titleH2, y = titleH1, title = "Habitat-dependent\nmovement\n") +
  theme(legend.position = "none",
...
p_DD <- ggplot(DD, aes(dens, K)) +
  ...
  labs(x = titleD2, y = titleD1, title = "Competition-driven\nmovement\n") +
  theme(legend.position = "none",
        axis.ticks = element_line(colour = "black", size = 0.25),
        panel.border = element_blank())
...
ggsave("./plots/final/FigS3_move_weights.png", width = 12, height = 6.15, dpi = dpi)
```
...
df_s7 <- df_infmean_plots %>% 
  ...
  mutate(
    ...
    threshold = factor(threshold, levels = c("> 2.5 years", "> 5 years", "> 7.5 years", "> 10 years")))
...
if (save == T) { ggsave("./plots/final/FigS7_pers_cfr_all.png", width = 7, height = 8, dpi = dpi) }
```
...
(fig_s8 <- ggplot(filter(df_s8, duration_quarter != 50), aes(duration_quarter / 4, prop)) + 
...
if (save == T) { ggsave("./plots/final/FigS8_time2extinction.png", width = 12, height = 7.1, dpi = dpi) }
```
...
(fig_s9 <- df_trans %>% 
  ...
  ggplot(aes(quarter / 4, avg)) +
    ...
    facet_grid(scenario ~ roaming) +
    scale_x_continuous(breaks = seq(0, 10, by = 2.5), labels = seq(0, 10, by = 2.5)) +
    scale_y_continuous(limits = c(-0.001, 0.25), breaks = seq(0, 0.3, by = 0.1)) +
...
if (save == T) { ggsave("./plots/final/FigS9_dynamics_g+m.png", width = 9, height = 6.7, dpi = dpi) }
```


## Fig. S10: Multipanel movement + landscape consequences (boxplot + jitter)

```{r fig-s10-MultiBoxJitter, fig.width = 11, fig.height = 16}
## a) density of hosts
fig_s10a <- ggplot(df_m1, aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
  geom_jitter(position = position_jitterdodge(dodge.width = 0.85, jitter.width = 0.15), 
              alpha = 0.1, size = 0.25) + 
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.75, position = position_dodge(0.85), 
               color = "grey25", alpha = 0, outlier.alpha = 0, size = 0.4, show.legend= F) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 175), breaks = seq(0, 150, by = 50)) +
  scale_color_manual(values = c("#69bdb5", "#766ce3"), name = "Group:") + 
  scale_shape_manual(values = c(19, 17), name = "Group:") + 
  guides(colour = guide_legend(override.aes = list(alpha = 0.75, size = 4))) +
  labs(x = "Landscape scenario", y = "\nVariance in\nhost density", 
       tag = "a", title = NULL, subtitle = NULL) +
  theme(axis.ticks.x = element_blank(),
        axis.text.x = element_blank(),
...
## b) visits by ranging males
fig_s10b <- ggplot(df_m2, aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
  geom_jitter(width = 0.175, alpha = 0.1, size = 0.25, shape = 19) + 
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.7,
               color = "grey25", alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 12.5), breaks = seq(0, 12, by = 6)) +
  scale_color_manual(values = c("#696969"), guide = F) + 
  scale_shape_manual(values = c(19, 17, 15), guide = F) + 
...
## c) contact rates per quality class
fig_s10c <- ggplot(df_m3, aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
  geom_jitter(position = position_jitterdodge(dodge.width = 0.75, jitter.width = 0.15), 
              alpha = 0.1, size = 0.25) + 
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.75, 
               position = position_dodge2(0.6, preserve = "single"), 
               color = "grey25", alpha = 0, outlier.alpha = 0, size = 0.4, show.legend = F) + 
  facet_grid(~ roaming) +
...
## d) week of maximum spread
fig_s10d <- ggplot(df_m4, aes(x = scenario, y = max, group = type, color = type, shape = type)) +   
  geom_jitter(width = 0.175, alpha = 0.5, size = 0.8) + 
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.7,
               color = "grey25", alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 55), breaks = seq(0, 50, by = 25)) +
  scale_color_manual(values = c("#b22222"), guide = F) + 
  scale_shape_manual(values = c(19), guide = F) + 
  labs(x = "Landscape scenario", y = "\nWeek of maximum\npathogen spread", 
       tag = "d", title = NULL, subtitle = NULL) +
  theme(axis.ticks.x = element_blank(),
...
## e) maximum number of infectious cells
fig_s10e <- ggplot(df_m5, aes(x = scenario, y = max, group = type, color = type, shape = type)) +   
  geom_jitter(width = 0.175, alpha = 0.5, size = 0.8) + 
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.7,
               color = "grey25", alpha = 0, outlier.alpha = 0, size = 0.4) + 
  facet_grid(~ roaming) +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.5)) +
  scale_color_manual(values = c("#ff7400"), guide = F) + 
  scale_shape_manual(values = c(19), guide = F) + 
  labs(x = "Landscape scenario", y = "\nMaximum proportion\nof infected patches", 
       tag = "e", title = NULL, subtitle = NULL) +
  theme(axis.ticks.x = element_blank(),
...
## f) trans within + between
fig_s10f <- ggplot(df_m6, aes(x = scenario, y = avg, group = type, color = type, shape = type)) +   
  geom_jitter(position = position_jitterdodge(dodge.width = 0.85, jitter.width = 0.15), 
              alpha = 0.1, size = 0.25) + 
  geom_boxplot(aes(group = interaction(scenario, type)), width = 0.75, position = position_dodge(0.85), 
               color = "grey25", alpha = 0, outlier.alpha = 0, size = 0.4, show.legend = F) + 
...
((fig_s10 <- fig_s10a + fig_s10b + fig_s10c + fig_s10d + fig_s10e + fig_s10f) *
  ...
  plot_layout(ncol = 1, height = c(1, 0.99, 0.99, 0.99, 0.99, 1)))

## final panel
if (save == T) { ggsave("./plots/final/FigS10_multi_raw.png", width = 11, height = 16, dpi = 350) }
```
